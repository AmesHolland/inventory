<template>
  <div>
    ...
    <el-table
      ...
      <el-table-column prop="deId" label="出货仓库" width="100"></el-table-column>
      ...
      <el-table-column label="操作" width="350px" align="center">
        <template slot-scope="scope" style="text-align: center">
          <el-popconfirm
              ...
              icon-color="red"
              title="确定入库吗？"
              @confirm="handleOutbound(scope.row,'已处理')"
          >
            <el-button v-if="(staff.role === '仓库管理员' || staff.role === '管理员') && scope.row.procState === '未处理'" type="success" slot="reference">一键出库<i class="el-icon-success"></i></el-button>
          </el-popconfirm>
      ...
      this.$request.post("/ob_order",this.form)
          .then(res =>{
            if(res.code === '200'){
              ...
              this.$message.error("出库失败，请等待货物补齐");
            }
          })
    },
    handleDel(id){
